...
EdgeExtendWidget::EdgeExtendWidget(DrawModelDlg *parent)
    : ElemWidgetObject(parent)
{
    ui.setupUi(this);
}

EdgeExtendWidget::~EdgeExtendWidget()   
{
}

bool EdgeExtendWidget::doModelOperation(float* wcsMatrix, const QString& name)
{ 
    ...
    double length = ui.lineEdit_x2->text().toDouble();
    ...
    if (builder.edgeExtendLength(origEntity, param, length, name.toStdString().c_str(), strPID, entity))
    {
        m_newNames << name;
...
void EdgeExtendWidget::on_toolButton_update_clicked()
{
	QString sourceStr = ui.lineEdit_PIDs->text();
	QStringList PIDs = sourceStr.split(",");
	if (PIDs.size() < 1)
